# This file is based on spinup/utils/run_utils.py.
# It additionally treats the different goals in the experiment grid like different seeds are treated.
...
def setup_logger_kwargs(exp_name, seed=None, goal=None, data_dir=None, datestamp=False):
    """
    Sets up the output_dir for a logger and returns a dict for logger kwargs.

    If no seed or goal is given and datestamp is false,

    ::

        output_dir = data_dir/exp_name

    If a seed or goal is given and datestamp is false,

    ::

        output_dir = data_dir/exp_name/exp_name_s[seed]_g[goal]

    If datestamp is true, amend to

    ::

        output_dir = data_dir/YY-MM-DD_exp_name/YY-MM-DD_HH-MM-SS_exp_name_s[seed]_g[goal]
    ...
    Args:
        ...
        goal (int, int): Goal to accomplish in environment.
    ...
    if seed is not None and goal is not None:
        # Make a seed- and goal-specific subfolder in the experiment directory.
        if datestamp:
            hms_time = time.strftime("%Y-%m-%d_%H-%M-%S")
            subfolder = ''.join([hms_time, '-', exp_name, '_s', str(seed), 'gx', "{:.2f}".format(goal[0]), 'gy',
                                 "{:.2f}".format(goal[1])])
        else:
            subfolder = ''.join(
                [exp_name, '_s', str(seed), 'gx', "{:.2f}".format(goal[0]), 'gy', "{:.2f}".format(goal[1])])
        relpath = osp.join(relpath, subfolder)
    elif goal is not None:
        # Make a goal-specific subfolder in the experiment directory.
        if datestamp:
            hms_time = time.strftime("%Y-%m-%d_%H-%M-%S")
            subfolder = ''.join(
                [hms_time, '-', exp_name, 'gx', "{:.2f}".format(goal[0]), 'gy', "{:.2f}".format(goal[1])])
        else:
            subfolder = ''.join([exp_name, 'gx', "{:.2f}".format(goal[0]), 'gy', "{:.2f}".format(goal[1])])
        relpath = osp.join(relpath, subfolder)
...
def call_experiment(exp_name, thunk, seed=0, goal=None, num_cpu=1, data_dir=None,
    ...
    # Send random seed and goal to thunk
    kwargs['seed'] = seed
    kwargs['goal'] = goal
    ...
    if 'logger_kwargs' not in kwargs:
        kwargs['logger_kwargs'] = setup_logger_kwargs(exp_name, seed, goal, data_dir, datestamp)
    else:
    ...
    entrypoint = osp.join(osp.abspath(osp.dirname(__file__)), '../../utils/run_entrypoint.py')
...
class ExperimentGrid:
    ...
    def print(self):
        ...
        # Count up the number of variants. The number counting seeds and goals
        # is the total number of experiments that will run; the number not
        # counting seeds and goals is the total number of otherwise-unique configs
        # being investigated.
        nvars_total = int(np.prod([len(v) for v in self.vals]))
        print(' Variants, counting seeds and goals: '.ljust(40), nvars_total)
        if 'seed' in self.keys:
            num_seeds = len(self.vals[self.keys.index('seed')])
            nvars_total = int(nvars_total / num_seeds)
        if 'goal' in self.keys:
            num_goals = len(self.vals[self.keys.index('goal')])
            nvars_total = int(nvars_total / num_goals)
        print(' Variants, not counting seeds and goals: '.ljust(40), nvars_total)
        print()
        print('=' * DIV_LINE_WIDTH)
    ...
    def variant_name(self, variant):
        ...
        Note: if ``seed`` or ``goal`` is a parameter, it is not included in the name.
        """
        ...
        for k, v, sh, inn in zip(self.keys, self.vals, self.shs, self.in_names):
            ...
            # Except, however, when the parameter is 'seed' or 'goal'. Seed or goal is handled
            # differently so that runs of the same experiment, with different 
            # seeds or goals, will be grouped by experiment name.
            if (len(v) > 1 or inn) and not (k == 'seed') and not (k == 'goal'):
    ...
    def run(self, thunk, num_cpu=1, data_dir=None, datestamp=False):
        ...
        to the args for call_experiment. However, ``seed`` and ``goal`` is omitted because
